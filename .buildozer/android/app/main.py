...
class DatabaseManager:
    def __init__(self):
        self.conn_str = NEON_CONN
        self.conn = None
        Logger.info("DatabaseManager: Using Neon PostgreSQL")

    async def _get_conn(self):
        if self.conn is None or self.conn.closed:
            self.conn = await psycopg.AsyncConnection.connect(self.conn_str)
        return self.conn
    ...
    async def get_specialties(self):
        try:
            ...
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, nombre FROM especialidades ORDER BY nombre")
                rows = await cur.fetchall()
                return [{"id": r[0], "nombre": r[1]} for r in rows]
        except Exception as e:
            Logger.error(f"DB Error (get_specialties): {e}")
            raise e
    ...
    async def get_institutions_by_specialty(self, specialty_id):
        try:
            ...
            async with conn.cursor() as cur:
                ...
                WHERE ie.especialidad_id = %s
                ORDER BY i.nombre, h.dia_num, h.hora_inicio
                """
                await cur.execute(query, (specialty_id,))
                rows = await cur.fetchall()
                ...
                for row in rows:
                    inst_id = row[0]
                    data[inst_id]["info"] = {
                        "id": inst_id, "nombre": row[1], "direccion": row[2],
                        "telefono": row[3], "lat": row[4], "lng": row[5],
                    }
            ...
            Logger.error(f"DB Error (get_institutions): {e}")
            raise e
    ...
    async def add_institution(self, nombre, direccion, telefono, lat=None, lng=None):
        try:
            ...
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO instituciones (nombre, direccion, telefono, lat, lng) VALUES (%s, %s, %s, %s, %s)",
                    [nombre, direccion, telefono, lat, lng]
                )
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (add_institution): {e}")
            raise e
    ...
    async def update_institution(self, id, nombre, direccion, telefono, lat, lng):
        try:
            ...
            async with conn.cursor() as cur:
                await cur.execute(
                    "UPDATE instituciones SET nombre=%s, direccion=%s, telefono=%s, lat=%s, lng=%s WHERE id=%s",
                    [nombre, direccion, telefono, lat, lng, id]
                )
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (update_institution): {e}")
            raise e
    ...
    async def delete_institution(self, id):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM instituciones WHERE id=%s", [id])
                await cur.execute("DELETE FROM institucion_especialidad WHERE institucion_id=%s", [id])
                await cur.execute("DELETE FROM horarios_atencion WHERE institucion_id=%s", [id])
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (delete_institution): {e}")
            raise e

    async def add_professional(self, nombre, apellido, matricula=None, telefono=None):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO profesionales (nombre, apellido, matricula, telefono) VALUES (%s, %s, %s, %s)",
                    [nombre, apellido, matricula, telefono]
                )
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (add_professional): {e}")
            raise e

    async def update_professional(self, id, nombre, apellido, matricula, telefono):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute(
                    "UPDATE profesionales SET nombre=%s, apellido=%s, matricula=%s, telefono=%s WHERE id=%s",
                    [nombre, apellido, matricula, telefono, id]
                )
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (update_professional): {e}")
            raise e

    async def delete_professional(self, id):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM profesionales WHERE id=%s", [id])
                await cur.execute("DELETE FROM horarios_atencion WHERE profesional_id=%s", [id])
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (delete_professional): {e}")
            raise e

    async def add_specialty(self, nombre):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("INSERT INTO especialidades (nombre) VALUES (%s)", [nombre])
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (add_specialty): {e}")
            raise e

    async def update_specialty(self, id, nombre):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("UPDATE especialidades SET nombre=%s WHERE id=%s", [nombre, id])
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (update_specialty): {e}")
            raise e

    async def delete_specialty(self, id):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM especialidades WHERE id=%s", [id])
                await cur.execute("DELETE FROM institucion_especialidad WHERE especialidad_id=%s", [id])
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (delete_specialty): {e}")
            raise e

    async def link_institution_specialty(self, inst_id, spec_id):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute(
            ...
            Logger.error(f"DB Error (link_institution_specialty): {e}")
            raise e
    ...
    async def get_all_institutions(self):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, nombre, direccion, telefono, lat, lng FROM instituciones ORDER BY nombre")
                rows = await cur.fetchall()
                return [{"id": r[0], "nombre": r[1], "direccion": r[2], "telefono": r[3], "lat": r[4], "lng": r[5]} for r in rows]
        except Exception as e:
            Logger.error(f"DB Error (get_all_institutions): {e}")
            raise e
    ...
    async def get_all_professionals(self):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("SELECT id, nombre, apellido, matricula, telefono FROM profesionales ORDER BY apellido")
                rows = await cur.fetchall()
                return [{"id": r[0], "nombre": r[1], "apellido": r[2], "matricula": r[3], "telefono": r[4]} for r in rows]
        except Exception as e:
            Logger.error(f"DB Error (get_all_professionals): {e}")
            raise e
    ...
    async def get_all_specialties(self):
        try:
            ...
            Logger.error(f"DB Error (get_all_specialties): {e}")
            raise e

    async def get_schedules_by_professional(self, prof_id):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                query = """
                SELECT h.id, h.institucion_id, i.nombre, h.dia_num, h.hora_inicio, h.hora_fin
                FROM horarios_atencion h
                JOIN instituciones i ON i.id = h.institucion_id
                WHERE h.profesional_id = %s
                ORDER BY h.dia_num, h.hora_inicio
                """
                await cur.execute(query, (prof_id,))
                rows = await cur.fetchall()
                return [{
                    "id": r[0], "inst_id": r[1], "inst_nombre": r[2],
                    "dia_num": r[3], "inicio": r[4], "fin": r[5]
                } for r in rows]
        except Exception as e:
            Logger.error(f"DB Error (get_schedules_by_professional): {e}")
            raise e

    async def add_schedule(self, prof_id, inst_id, dia_num, inicio, fin):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute(
                    "INSERT INTO horarios_atencion (profesional_id, institucion_id, dia_num, hora_inicio, hora_fin) VALUES (%s, %s, %s, %s, %s)",
                    [prof_id, inst_id, dia_num, inicio, fin]
                )
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (add_schedule): {e}")
            raise e
    ...
    async def delete_schedule(self, id):
        try:
            conn = await self._get_conn()
            async with conn.cursor() as cur:
                await cur.execute("DELETE FROM horarios_atencion WHERE id=%s", [id])
                await conn.commit()
        except Exception as e:
            Logger.error(f"DB Error (delete_schedule): {e}")
            raise e

def create_db_manager():
    """Factory: returns the DatabaseManager."""
    if psycopg is None:
        raise ImportError("psycopg not found. Please install it.")
